...
- Scene-segmentation call token usage is **not recorded** -- `scenes.json` stores only `segments`/`segment_count`/`model`, unlike `genre.json`'s `input_tokens`/`output_tokens` fields. This is a real gap in `scene_analysis.py`'s `make_segment_extractor` for this item's own cost-visibility goal (Required Changes #5); not fixed here (out of this evaluation-only item's scope), but worth a follow-up.
